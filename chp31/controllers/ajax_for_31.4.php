...
if (array_key_exists('AjaxAddUpdateTask', $_POST)) { 
		...
		if ($task_id != 0) {
			...
			if ($user_id_for_task != constant('USER_ID')) {
				$ServerSideCheckErrors[]="That's not your task";
			}
		...
		if ( mysqli_num_rows($get_entries) == 0) {
			...
			<?php
				while ($entries = mysqli_fetch_array($get_entries)) {
					?>
						<tr id="task_id_<?php print $entries['id'];?>">
							...
							<td><a href="#" onClick="UpdateShowAllFieldsTask('<?php print $entries['id'];?>'); return false;">Edit</a></td>
							<td><a href="#" onClick="DeleteTask('<?php print $entries['id'];?>'); return false;">Delete</a></td>
						</tr>
					<?php
				} // while
			?>
...
if (array_key_exists('AjaxUpdateTaskAllFields', $_POST)) { 
	...
	<form>	
		...
		<div>Description:</div>
		<div><input type="text" 
				name="x<?php print $task_id;?>_description" 
				id="x<?php print $task_id;?>_description" 
				value="<?php print $TaskInfo['description'];?>" 
				class="form-control" />
		</div>

		<div>Date Assigned:</div>
		<div><input type="text" 
					name="x<?php print $task_id;?>_date_assigned" 
					id="x<?php print $task_id;?>_date_assigned" 
					value="<?php print date("n/j/Y", strtotime($TaskInfo['date_assigned']));?>" 
					class="form-control" />
		</div>

		<div>Completed: Required</span></div>
		<div>
			<select name="x<?php print $task_id;?>_completed" 
					id="x<?php print $task_id;?>_completed" 
					class="form-control">

				<option value=""></option>
				<option value="Y" 
					<?php if ($TaskInfo['completed'] == "Y") { ?> 
						selected="selected" 
					<?php } ?>
				 >Yes
				</option>
				<option value="N" 
					<?php if ($TaskInfo['completed'] == "N") { ?> 
						selected="selected" 
						<?php } ?>
				 >No</option>
			</select>
		...
		<div style="margin-top:10px;">
			<input type="button" 
					name="btUpdateTask" 
					onClick="AddUpdateTask('<?php print $task_id;?>');" 
...
if (array_key_exists('AjaxDeleteTask', $_POST)) {
