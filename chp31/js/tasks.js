	...
	$.post( 
		...
		function(responseText){
			...
			if (responseText == "task-added") {
				...
				setTimeout(function(){ $('#task_updated_confirmation').css("visibility","hidden"); }, 3000);
			}
...
function UpdateShowAllFieldsTask(task_id) {

	$('#task_id_' + task_id).html(ajax_load);

	$.post( 
		ajaxController,

		{AjaxUpdateTaskAllFields:1,  task_id: task_id},

		function(responseText){

			$('#task_id_' + task_id).html(responseText);
...
function DeleteTask(task_id) {
	...
	$.post( 
		...
		{AjaxDeleteTask:1,  task_id: task_id},
